...
class MSAClient:
    ...
    def __init__(self):
        # config.py의 settings에서 서비스 URL 읽기
        self.service_urls = {
    ...
    async def _make_request(
        ...
        try:
            logger.debug(f"MSA 요청: {method} {url} (data: {data}, params: {params})")
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.debug(f"MSA 응답 성공: {url} - {len(str(result))} bytes")
                    return result
            ...
            logger.error(f"연결 실패 (서비스가 실행 중인지 확인하세요): {url} - {str(e)}")
            return None
        except httpx.TimeoutException:
            logger.error(f"Request timeout: {url}")
            return None
        except Exception as e:
            logger.error(f"Request error: {url} - {str(e)}", exc_info=True)
            return None
    ...
    async def call_ai_meeting_start(self, data: Dict[str, Any]) -> Optional[Dict]:
        """AI 서비스 - 회의 시작"""
        return await self._make_request("ai", "/ai/meeting/start", "POST", data)
    
    async def call_ai_meeting_end(self, data: Dict[str, Any]) -> Optional[Dict]:
        """AI 서비스 - 회의 종료"""
        return await self._make_request("ai", "/ai/meeting/end", "POST", data)
    
    async def call_ai_minutes_generate(self, data: Dict[str, Any]) -> Optional[Dict]:
        """AI 서비스 - 회의록 생성"""
        return await self._make_request("ai", "/ai/minutes", "POST", data)

    # =================================================================
    # Team Service API 호출 (자기 자신 - 필요시 사용)
    # =================================================================

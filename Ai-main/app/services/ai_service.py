...
class AiService:
    ...
    async def generate_questions(self, request: QuestionRequest, repo: TestRepository, user_id: str) -> QuestionResponse:
        ...
        if request.type == "MIXED":
            ...
            # DB 문제를 QuestionItem으로 변환 (주관식은 채점 기준 보강)
            db_mc_items = [QuestionItem(**q.question_json) for q in db_mc_questions]
            db_sa_items = []
            for q in db_sa_questions:
                q_data = q.question_json.copy()
                # 기존 DB 문제에 채점 기준이 없으면 기본값 추가
                if 'grading_criteria' not in q_data or not q_data['grading_criteria']:
                    q_data['grading_criteria'] = "핵심 개념 이해도와 실무 적용력을 평가. 유사 표현도 정답으로 인정."
                if 'acceptable_keywords' not in q_data or not q_data['acceptable_keywords']:
                    # answer에서 키워드 추출
                    answer_text = str(q_data.get('answer', ''))
                    q_data['acceptable_keywords'] = [kw.strip() for kw in answer_text.replace(',', ' ').split() if len(kw.strip()) > 1]
                db_sa_items.append(QuestionItem(**q_data))
    ...
    async def _generate_questions_from_ai(self, stack: str, difficulty: str, count: int, q_type: str = "MULTIPLE_CHOICE", target_mc: int = 3, target_sa: int = 2) -> list[QuestionItem]:
        ...
        system_prompt = "당신은 실무 중심의 프로젝트 테크 리드(Tech Lead)입니다. 이론적인 지식보다는 실제 개발 상황에서의 문제 해결 능력과 트러블 슈팅 역량을 검증하는 데 집중하세요. 단, 난이도는 기초~중급 수준으로 유지하여 지원자가 부담 없이 풀 수 있도록 하세요. 코드 스니펫은 절대 포함하지 마세요. JSON 배열만 출력합니다."
...
- 난이도: 기초~중급 수준 (지원자가 부담 없이 풀 수 있는 수준)
- 코드 스니펫 없이 텍스트로만 출제

[객관식 형식]
{{
  "type": "MULTIPLE_CHOICE",
  "question": "질문 내용 (코드 없이 텍스트로만, 기초~중급 난이도)",
...
{{
  "type": "SHORT_ANSWER",
  "question": "상황을 설명하고 해결책을 물어보는 질문 (코드 없이 텍스트로만, 기초~중급 난이도)",
  "options": [],
  "answer": "모범 답안 (핵심 키워드 2-3개 포함)",
  "explanation": "해설",
  "grading_criteria": "정답으로 인정되는 핵심 키워드: [키워드1, 키워드2]. 이 중 1개 이상 포함 시 정답 처리. 유사 표현도 정답으로 인정.",
  "acceptable_keywords": ["키워드1", "키워드2", "유사표현1", "유사표현2"]
}}
...
"""
            ...
            기초~중급 난이도의 실무 중심 '주관식(단답형)' 문제 {count}개를 한국어로 생성하세요.
            
            [출제 가이드]
            1. 특정 상황에서의 해결책이나 개념을 서술하게 하세요.
            2. 코드 스니펫은 절대 포함하지 마세요. 텍스트로만 질문하세요.
            3. 정답은 명확한 키워드나 핵심 개념이 포함되어야 합니다.
            4. 난이도는 기초~중급 수준으로, 해당 기술을 1년 정도 사용한 개발자가 풀 수 있는 수준.
            
            [출력 형식]
            JSON 배열 (각 객체는 question, answer, explanation, grading_criteria, acceptable_keywords 포함)
            - type: "SHORT_ANSWER" (필수)
            - question: 구체적인 상황 설명 질문 (코드 없이 텍스트로만, 한국어)
            - answer: 모범 답안 (핵심 키워드 포함 필수)
            - explanation: 해설 및 실무 팁
            - grading_criteria: "정답으로 인정되는 핵심 키워드: [키워드1, 키워드2]. 이 중 1개 이상 포함 시 정답 처리."
            - acceptable_keywords: ["정답키워드1", "정답키워드2", "유사표현1", "유사표현2"] (정답으로 인정할 키워드 목록)
            - options: [] (빈 리스트)
            ...
            user_prompt = f"""
            '{stack}' 기술을 사용하여 프로젝트를 진행할 예비 팀원을 선발하려 합니다.
            기초~중급 난이도의 실무 중심 '객관식' 문제 {count}개를 한국어로 생성하세요.
            ...
            3. 코드 스니펫은 절대 포함하지 마세요. 텍스트로만 질문하세요.
            4. 난이도는 기초~중급 수준으로, 해당 기술을 1년 정도 사용한 개발자가 풀 수 있는 수준.
            ...
            for item in data:
                ...
                if item['type'] == 'SHORT_ANSWER':
                    ...
                    if 'grading_criteria' not in item:
                        item['grading_criteria'] = "핵심 개념 이해도와 실무 적용력을 평가. 유사 표현도 정답으로 인정."
                    if 'acceptable_keywords' not in item:
                        # answer에서 키워드 추출하여 기본값 설정
                        answer_text = str(item.get('answer', ''))
                        item['acceptable_keywords'] = [kw.strip() for kw in answer_text.split(',') if kw.strip()]
    ...
    async def generate_minutes_from_chat(self, chat_text: str, attendees: list = None, meeting_date: str = None) -> dict:
        """회의록 생성: 채팅 텍스트를 분석하여 구조화된 회의록 JSON 반환"""
        from datetime import datetime
        
        # 날짜가 없으면 오늘 날짜 사용
        final_date = meeting_date if meeting_date else datetime.now().strftime("%Y-%m-%d")
        
        # 참석자 목록 문자열 생성
        attendees_str = ", ".join(attendees) if attendees else "채팅 로그에서 추출"
    ...
    - Meeting Date: {final_date}
    - Attendees: {attendees_str}
    
    JSON Structure: {{
        "date": "{final_date}",
        "attendees": {json.dumps(attendees, ensure_ascii=False) if attendees else '["채팅 로그에서 이름 추출"]'},
        "agenda": "Main Topic",
    ...
    [Rules]
    1. The "date" field MUST be exactly "{final_date}".
    2. The "attendees" field MUST only include: {attendees_str}. Do NOT add anyone else.
    3. All text should be in Korean.
    ...
    """
        ...
        try:
             ...
             # 안전장치: AI가 날짜나 참석자를 변경했을 경우 강제로 덮어쓰기
             result["date"] = final_date
             if attendees:
    ...
    async def grade_answer(self, question: str, user_answer: str, model_answer: str, criteria: str, acceptable_keywords: list = None) -> dict:
        """
        주관식 답변 AI 채점 (유사 답변 정답 처리)
        """
        keywords_hint = ""
        if acceptable_keywords:
            keywords_hint = f"\n[정답으로 인정되는 키워드 목록] {', '.join(acceptable_keywords)}"
        
        system_prompt = "당신은 관대하고 공정한 기술 면접관입니다. 사용자가 핵심 개념을 이해하고 있다면 표현이 다르더라도 정답으로 인정합니다. JSON으로 반환하세요."
        user_prompt = f"""
        다음 주관식 문제에 대한 사용자의 답안을 채점해주세요.
        
        [문제] {question}
        [모범 답안] {model_answer}
        [채점 기준] {criteria}{keywords_hint}
        [사용자 답안] {user_answer}
        
        [채점 원칙 - 중요!]
        1. 사용자 답안이 모범 답안과 완전히 일치하지 않아도 됩니다.
        2. 핵심 키워드나 개념이 포함되어 있으면 정답으로 처리합니다.
        3. 유사한 의미의 표현(동의어, 다른 용어)도 정답으로 인정합니다.
           예: "캐싱" = "캐시" = "cache" = "임시저장"
           예: "비동기" = "async" = "논블로킹" = "non-blocking"
        4. 오타나 띄어쓰기 차이는 무시합니다.
        5. 부분 정답도 인정합니다 (여러 키워드 중 일부만 맞아도 부분 점수).
        
        [점수 기준]
        - 90-100점: 핵심 개념을 정확히 이해하고 설명함
        - 70-89점: 핵심 개념을 이해하고 있으나 표현이 부족함 (정답 처리)
        - 50-69점: 부분적으로 이해하고 있음 (부분 정답)
        - 0-49점: 핵심 개념을 이해하지 못함
        
        [출력 형식 (JSON)]
        {{
            "score": 85,
            "is_correct": true,
            "feedback": "핵심 개념인 A를 잘 이해하고 있습니다. (추가 설명)"
        }}
        
        is_correct는 score가 70점 이상이면 true입니다.
        """
        try:
            res_text = await self._invoke_bedrock(system_prompt, user_prompt)
            cleaned = res_text.strip().replace("```json", "").replace("```", "").strip()
            result = json.loads(cleaned)
            # 70점 이상이면 is_correct = true 강제

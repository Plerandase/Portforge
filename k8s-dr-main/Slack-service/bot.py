...
ERROR_RATE_THRESHOLD = int(os.environ.get("ERROR_RATE_THRESHOLD", "5"))  # 5%로 낮춤 (5xx만 계산하므로)
...
def get_all_metrics() -> str:
    """모든 서비스 메트릭 조회 (향상된 버전)"""
    result = f"📈 *서비스 메트릭 리포트* ({datetime.now().strftime('%Y-%m-%d %H:%M:%S')})\n\n"

...
export const authAPI = {
    // [추가] 소셜 로그인 콜백 (Code -> Token)
    socialCallback: async (code: string): Promise<any> => {
        const response = await apiFetch('/auth/social/callback', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }, // Added missing header for JSON body
            body: JSON.stringify({ code }),
        });
        if (!response.ok) {
            const error = await response.json().catch(() => ({}));
            throw new Error(error.message || error.detail || '소셜 로그인 처리에 실패했습니다.');
        }

...
class MSAClient:
    ...
    async def get_test_result_by_application(self, application_id: int) -> Optional[Dict]:
        """지원서별 테스트 결과 조회"""
        return await self._make_request("ai", f"/test-results/{application_id}")
    
    async def get_user_test_results(self, user_id: str) -> Optional[List[Dict]]:
        """사용자별 테스트 결과 목록 조회"""
        return await self._make_request("ai", f"/test-results/user/{user_id}")
    ...
    async def get_team_reports(self, team_id: int, report_type: Optional[str] = None) -> Optional[List[Dict]]:
        ...
        return await self._make_request("ai", f"/reports/team/{team_id}", params=params)
    
    async def get_report_detail(self, report_id: int) -> Optional[Dict]:
        """리포트 상세 정보 조회"""
        return await self._make_request("ai", f"/reports/{report_id}")
    ...
    async def get_user_portfolios(self, user_id: str, is_public: Optional[bool] = None) -> Optional[List[Dict]]:
        ...
        return await self._make_request("ai", f"/portfolios/user/{user_id}", params=params)

    # =================================================================
    # Team Service API 호출 (추후 구현)
    # =================================================================

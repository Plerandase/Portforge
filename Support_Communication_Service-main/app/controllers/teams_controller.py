    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.post(
                f"{settings.TEAM_SERVICE_URL}/api/v1/teams/{project_id}/meeting_trigger",
            ...
            return ResponseEnvelope(
                success=True, 
                code="TEAM_004", 
                message=f"Meeting {payload.action}", 
                data=result
            )
        ...
        return ResponseEnvelope(
            success=False, 
            code="TEAM_004_ERR", 
            message=f"Failed to trigger meeting: {str(e)}", 
            data={"project_id": project_id, "action": payload.action}
        )
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{settings.TEAM_SERVICE_URL}/api/v1/teams/{project_id}/meetings",
                timeout=10.0
            )
            response.raise_for_status()
        ...
        # 실패 시 빈 목록 반환
        return ResponseEnvelope(success=True, code="TEAM_005", message="Meetings", data=[])
...
async def create_meeting(project_id: int, payload: MeetingRequest, current_user=Depends(get_current_user)):
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            response.raise_for_status()
            # Team Service 응답을 그대로 반환 (FE가 meeting.session_id 접근 가능)
            return JSONResponse(content=response.json(), status_code=response.status_code)
    except httpx.HTTPError as e:
        return JSONResponse(
            content={"success": False, "message": f"Failed to create meeting: {str(e)}"},
            status_code=500
        )
